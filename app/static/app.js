...
form.addEventListener('submit', async (event) => {
  ...
  try {
    ...
    fallback.textContent = isFallback
      ? `Generation unavailable: ${data.fallback_reason}. The passages below are still usable source evidence.`
      : '';
...
uploadForm.addEventListener('submit', async (event) => {
  ...
  try {
    ...
    const chunks =
      data.stats && data.stats.chunks !== undefined
        ? data.stats.chunks
        : '?';
...
function showUploadStatus(message, success) {
  ...
  uploadStatus.style.color = success
    ? '#4ade80'
    : '#ef4444';
}